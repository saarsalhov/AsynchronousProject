...
router.post("/addCostItem", async function (req, res) {
  ...
  dbConnect
    ...
    .insertOne(newCostItem, function (err, response) {
      if (err) {
            res.status(400).send("Error fetching addCostItem")
        }
      else {
          ...
          dbConnect
              ...
              if (err) {
                ...
                dbConnect.db("async_course_db").collection("costs_by_month").updateOne(myQuery, newValues, function(err, res) {
                  ...
                  res.status(200).send(JSON.stringify({ status: "item added to costs + append to costs by mont" }));
              } else {
                ...
                res.status(200).send(JSON.stringify({ status: "item added to costs + add to costs by mont" }));
              }
            });
      }
    });
